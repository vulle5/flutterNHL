class Player {
  ...
  factory Player.fromJson(Map<String, dynamic> parsedJson) {
    ...
    return Player(
        ...
        primaryPosition:
            PrimaryPosition.fromJson(parsedJson['primaryPosition']),
        careerRegularSeason:
            CareerRegularSeason.fromJson(parsedJson['careerRegularSeason']),
...
class CurrentTeam {
  ...
  const CurrentTeam({this.id, this.name, this.link});
...
class PrimaryPosition {
  ...
  const PrimaryPosition({this.code, this.name, this.type, this.abbreviation});
...
class CareerRegularSeason {
  ...
  factory CareerRegularSeason.fromJson(Map<String, dynamic> parsedJson) {
    ...
    return CareerRegularSeason(
        timeOnIce: parsedJson['timeOnIce'] ?? "N/A",
        ...
        powerPlayTimeOnIce: parsedJson['powerPlayTimeOnIce'] ?? "N/A",
        evenTimeOnIce: parsedJson['evenTimeOnIce'] ?? "N/A",
        penaltyMinutes: parsedJson['penaltyMinutes'] ?? "N/A",
        ...
        shortHandedTimeOnIce: parsedJson['shortHandedTimeOnIce'] ?? "N/A",
        blocked: parsedJson['blocked'] ?? 0,
        plusMinus: parsedJson['plusMinus'] ?? 0,
        points: parsedJson['points'] ?? 0,
        shifts: parsedJson['shifts'] ?? 0,
        timeOnIcePerGame: parsedJson['timeOnIcePerGame'] ?? "N/A",
        evenTimeOnIcePerGame: parsedJson['evenTimeOnIcePerGame'] ?? "N/A",
        shortHandedTimeOnIcePerGame:
            parsedJson['shortHandedTimeOnIcePerGame'] ?? "N/A",
        powerPlayTimeOnIcePerGame:
            parsedJson['powerPlayTimeOnIcePerGame'] ?? "N/A");
  }
}
...
class YearByYear {
  ...
  factory YearByYear.fromJson(Map<String, dynamic> parsedJson) {
    ...
    List<Season> seasonList =
        list.map((item) => Season.fromJson(item)).toList();

    return YearByYear(seasons: seasonList);
  }
}
...
class League {
  ...
  const League({this.name, this.link});

  factory League.fromJson(Map<String, dynamic> parsedJson) {
    if (parsedJson == null) return null;

    return League(name: parsedJson['name'], link: parsedJson['link']);
  }
}
